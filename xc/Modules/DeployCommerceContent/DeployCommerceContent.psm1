...
Function Invoke-DeployCommerceContentTask {
	...
	param(
		...
		[Parameter(Mandatory=$false)]
		[string]$BizFxServiceUrl,
		[Parameter(Mandatory=$false)]
		[string]$CommerceAuthoringServiceUrl
	)
	...
	try {       
		switch ($Name) {
			{($_ -match "CommerceOps")} {                    
				...
				$bizfx = "https://localhost:$SitecoreBizFxPort"
				if ($CommerceServicesHostPostfix){
					$bizfx = $BizFxServiceUrl
				}
				$allowedOrigins = @("$($bizfx)", "https://$SiteHostHeaderName")
				...
				foreach ($p in $originalJson.Policies.'$values') {
					if ($p.'$type' -eq 'Sitecore.Commerce.Plugin.SQL.EntityStoreSqlPolicy, Sitecore.Commerce.Plugin.SQL') {						
						...
						$p.UserName = $SqlAdminUser
						$p.Password = $SqlAdminPassword
					...
					} elseif ($p.'$type' -eq 'Plugin.Sample.SideBySideUpgrade.MigrationSqlPolicy, Plugin.Sample.SideBySideUpgrade') {
						...
						$p.SourceStoreSqlPolicy.UserName = $SqlAdminUser
						$p.SourceStoreSqlPolicy.Password = $SqlAdminPassword
				...
				foreach ($jsonFile in $environmentFiles) {
					...
					foreach ($p in $json.Policies.'$values') {
						if ($p.'$type' -eq 'Sitecore.Commerce.Plugin.SQL.EntityStoreSqlPolicy, Sitecore.Commerce.Plugin.SQL') {
						...
						} elseif ($p.'$type' -eq 'Plugin.Sample.Customers.CsMigration.ProfilesSqlPolicy, Plugin.Sample.Customers.CsMigration') {
							$p.Server = $CommerceServicesDbServer
							$p.UserName = $SqlAdminUser
							$p.Password = $SqlAdminPassword
							$p.TrustedConnection = $false
						    ...
						    if ($CommerceServicesHostPostfix) {
								$p.ServiceUrl = $CommerceAuthoringServiceUrl;							
							} else {
			...
			{($_ -match "BizFx")} {
				...
				$originalJson = Get-Content $pathToJson -Raw  | ConvertFrom-Json
				if ($CommerceServicesHostPostfix){
					$originalJson.BizFxUri = $BizFxServiceUrl
					$originalJson.EngineUri = $CommerceAuthoringServiceUrl
				} else {
